...
inner join task
on Job_task.fk_task_id = task.task_id
where time(time_to_sec(job_task.start_time)) >=time_to_sec('05:00:00') and time(time_to_sec(job_task.start_time)) < time_to_sec('14:30:00') and date(job_task.end_time) = date
